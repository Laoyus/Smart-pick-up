...
void loop()
{
    ...
    {
        if (i == 2)
            continue; // bin 2 IR sensor not connected — skip to avoid spurious triggers
...
void handleCommand(const String &cmd)
{
    ...
    else if (cmd.startsWith("STREAM:"))
    {
        ...
        Serial.println(streamOn ? "STREAM ON" : "STREAM OFF");
        lastStreamMs = millis();
    }
}
